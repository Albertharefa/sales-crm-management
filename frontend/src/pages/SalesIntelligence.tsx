...
export default function SalesIntelligence() {
  ...
  return <div className="space-y-6" data-testid="sales-intelligence-page">
    ...
    <section className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 px-6 py-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><div className="flex items-center gap-2"><BarChart3 className="size-5 text-blue-600" /><h2 className="font-heading text-lg font-semibold">Monthly Sales Performance</h2></div><p className="text-xs text-slate-500">Actual PO value dibandingkan target bulanan pada scope user</p></div><div className="text-xs text-slate-400">Tahun berjalan</div></div></div><div className="grid gap-3 p-6 md:grid-cols-2 xl:grid-cols-4">{data.monthly_sales_performance.map((row) => { const actual = Number(row.actual) || 0; const monthlyTarget = Number(row.target) || 0; const achievementValue = monthlyTarget > 0 ? (actual / monthlyTarget) * 100 : 0; return <div key={row.month} className="rounded-xl border border-slate-100 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-slate-900">{row.label}</span><span className="text-xs font-semibold text-slate-500">{monthlyTarget > 0 ? percent(achievementValue) : "—"}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-blue-600" style={{ width: `${Math.min(100, (actual / maxMonthly) * 100)}%` }} /></div><div className="mt-3 flex items-center justify-between gap-3 text-[11px]"><span className="font-semibold text-slate-700">Won/PO {compact(actual)}</span><span className="text-slate-400">Target {compact(monthlyTarget)}</span></div></div>; })}</div></section>
