...
export default function PurchaseOrders() {
  ...
  const subtotal = items.reduce((sum, item) => sum + Math.max(0, Number(item.quantity || 0) * Number(item.unit_price || 0)), 0);
  ...
  return (
    <div data-testid="purchase-orders-page">
      ...
      <div className="mb-5 grid grid-cols-1 gap-3 md:grid-cols-[minmax(360px,1fr)_220px_260px]">
        <div className="relative w-full"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari nomor PO / customer..." className="h-10 w-full pl-9" data-testid="purchase-orders-search-input" /></div>
      ...
      {viewOrder && <Modal title={"Detail PO " + viewOrder.po_number} onClose={() => setViewOrder(null)} size="landscape">
        <div className="grid gap-5 sm:grid-cols-2"><div className="space-y-3 rounded-lg border border-slate-200 p-5"><div><span className="text-xs text-slate-500">NO PO CUSTOMER</span><div className="font-mono font-semibold">{viewOrder.po_number}</div></div><div><span className="text-xs text-slate-500">CUSTOMER</span><div>{viewOrder.customer_name}</div></div><div><span className="text-xs text-slate-500">TANGGAL</span><div>{viewOrder.date}</div></div><div><span className="text-xs text-slate-500">SALES</span><div>{viewOrder.sales_name ?? "—"}</div></div></div><div className="space-y-3 rounded-lg border border-slate-200 p-5"><div><span className="text-xs text-slate-500">STATUS</span><div><Badge>{viewOrder.status}</Badge></div></div><div><span className="text-xs text-slate-500">NILAI PO</span><div className="font-mono font-semibold">{money(viewGrandTotal)}</div></div><div><span className="text-xs text-slate-500">ETA</span><div>{viewOrder.eta ?? "—"}</div></div><div><span className="text-xs text-slate-500">SUPPLIER</span><div>{viewOrder.supplier ?? "—"}</div></div></div></div>
      ...
      {modal && <Modal title={editing ? "Edit Purchase Order Customer" : quotationId ? "Convert Quotation ke Purchase Order Customer" : "Input Purchase Order dari Customer"} onClose={closeForm} size="landscape">
        <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (!form.customer_id || !form.sales_id || !form.po_number || items.length === 0) { toast.error("Customer, Sales, No PO, dan minimal 1 item wajib diisi"); return; } if (items.some((item) => !item.description.trim() || Number(item.quantity) < 1)) { toast.error("Deskripsi item dan Qty wajib diisi"); return; } saveOrder.mutate(); }} data-testid="purchase-order-create-form">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4"><div className="mb-4 flex items-center justify-between"><div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Item PO</div><Button type="button" variant="outline" size="sm" onClick={() => setItems((current) => [...current, emptyItem()])} data-testid="purchase-order-add-item-button"><Plus className="mr-2 h-4 w-4" />Tambah Item</Button></div>
            <div className="space-y-3">{items.map((item, index) => { const itemSubtotal = Math.max(0, Number(item.quantity || 0) * Number(item.unit_price || 0)); const itemDiscount = Math.max(0, Number(item.discount || 0)); const itemTax = Math.max(0, itemSubtotal - itemDiscount) * (Number(item.tax || 0) / 100); const itemTotal = Math.max(0, itemSubtotal - itemDiscount) + itemTax; return <div key={index} className="rounded-lg border border-slate-200 bg-slate-50/60 p-4" data-testid={`purchase-order-item-${index}`}>
