...
export default function OrderMonitoring() {
  ...
  const update = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["order-monitoring"] }); qc.invalidateQueries({ queryKey: ["dashboard"] }); toast.success("Status order diperbarui"); },
  ...
  const customerByKey = useMemo(() => {
    ...
    customers.forEach((customer) => { map.set(customer.id, customer); if (customer.customer_id) map.set(customer.customer_id, customer); });
    return map;
  }, [customers]);
  ...
  const salesOptions = useMemo(() => {
    const fromSalesTeam = (salesQuery.data ?? []).map((item) => item.sales).filter(Boolean);
    const fromCustomers = customers.map((customer) => customer.sales_name).filter(Boolean) as string[];
    const fromOrders = linkedItems.map((item) => item.sales_name).filter(Boolean) as string[];
    return Array.from(new Set([...fromSalesTeam, ...fromCustomers, ...fromOrders])).sort();
  }, [salesQuery.data, customers, linkedItems]);
  ...
  const filteredItems = useMemo(() => {
    ...
    return linkedItems.filter((item) => {
      ...
      const indicator = etaIndicator(item.eta, item.status);
      ...
      const matchesEta = !etaFilter || indicator === etaFilter;
  ...
  return (
    <div data-testid="order-monitoring-page">
      ...
      <div className="mb-4 grid grid-cols-[360px_145px_175px_150px_245px] gap-2 overflow-x-auto border-y border-slate-200 bg-white p-3">
        <div className="relative w-[360px] max-w-full"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari PO / produk / customer..." className="h-10 w-full rounded-md border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none transition focus:border-blue-400 focus:ring-1 focus:ring-blue-100" /></div>
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className={`${selectClass} h-10 min-w-0`}><option value="">Semua status</option>{stages.map((stage) => <option key={stage} value={stage}>{stage}</option>)}</select>
        <select value={etaFilter} onChange={(event) => setEtaFilter(event.target.value)} className={`${selectClass} h-10 min-w-0`}><option value="">Semua indikator ETA</option><option value="Overdue">Overdue</option><option value="Due Soon">Due Soon</option><option value="Selesai">Selesai</option></select>
        <select value={salesFilter} onChange={(event) => setSalesFilter(event.target.value)} className={`${selectClass} h-10 min-w-0`}><option value="">Semua sales</option>{salesOptions.map((sales) => <option key={sales} value={sales}>{sales}</option>)}</select>
        <select value={customerFilter} onChange={(event) => setCustomerFilter(event.target.value)} className={`${selectClass} h-10 min-w-0`}><option value="">Semua customer</option>{customerOptions.map((customer) => <option key={customer} value={customer}>{customer}</option>)}</select>
      </div>
      ...
      <DataTable testId="order-monitoring-table" items={filteredItems} loading={query.isLoading || customerQuery.isLoading || salesQuery.isLoading} total={filteredItems.length} columns={[
        ...
        { key: "indicator", label: "Indikator", render: (item) => { const indicator = etaIndicator(item.eta, item.status); return <Badge variant={indicator === "Overdue" ? "destructive" : "outline"}>{indicator}</Badge>; } },
