        ...
        <section className="relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-orange-50/25 to-amber-50/35 p-5 sm:p-10">
          ...
          <div className="relative z-10 w-full max-w-md">
            <div className="mb-8 lg:hidden">
              <div className="flex items-center gap-3">
                <div className="flex size-10 items-center justify-center rounded-xl bg-orange-500 font-heading text-xl font-bold text-white">C</div>
            ...
            <div className="rounded-2xl border border-orange-100 bg-white/95 p-7 shadow-xl shadow-orange-100/50 backdrop-blur sm:p-9">
              ...
              <form onSubmit={submit} className="space-y-5" data-testid="login-form">
                ...
                <Field label="Password" required>
                  <div className="relative">
                    ...
                    <button type="button" className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 transition hover:text-orange-600" onClick={() => setShowPassword(v => !v)} title={showPassword ? "Sembunyikan password" : "Lihat password"} aria-label={showPassword ? "Sembunyikan password" : "Lihat password"} data-testid="login-password-toggle">
                      {showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
                    </button>
                  </div>
                </Field>
                <div className="flex justify-end -mt-2">
                  <button type="button" onClick={openForgotPassword} className="text-xs font-medium text-orange-600 transition hover:text-orange-800 hover:underline" data-testid="forgot-password-link">Lupa password?</button>
                </div>
                <Button type="submit" className="h-11 w-full bg-orange-600 hover:bg-orange-700" disabled={mutation.isPending} data-testid="login-submit-button">
              ...
              <div className="mt-7 border-t border-orange-100 pt-5">
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                  <button type="button" className="rounded border border-slate-200 px-2 py-1 hover:border-orange-300 hover:text-orange-600" onClick={() => setEmail("admin@crm.co.id")} data-testid="demo-admin-button">Super Admin</button>
                  <button type="button" className="rounded border border-slate-200 px-2 py-1 hover:border-orange-300 hover:text-orange-600" onClick={() => setEmail("manager@crm.co.id")} data-testid="demo-manager-button">Sales Manager</button>
                  <button type="button" className="rounded border border-slate-200 px-2 py-1 hover:border-orange-300 hover:text-orange-600" onClick={() => setEmail("sales@crm.co.id")} data-testid="demo-sales-button">Sales</button>
                </div>
              </div>
            </div>
          </div>
