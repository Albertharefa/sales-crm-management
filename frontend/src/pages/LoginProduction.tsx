...
function loginErrorMessage(error: unknown) {
  if (axios.isAxiosError(error)) {
    ...
    if (status === 423) return detail || "Akun terkunci sementara. Silakan coba lagi nanti.";
    if (status === 403) return detail || "Akun tidak aktif atau role tidak valid.";
    if (status === 401) return detail || "Email/User ID atau password salah.";
    return detail || "Login gagal. Silakan coba lagi.";
  }
  return "Login gagal. Silakan coba lagi.";
}

export default function LoginProduction() {
  ...
  const showDemo = import.meta.env.VITE_SHOW_DEMO_LOGIN === "true";

  const mutation = useMutation({
    mutationFn: () => apiPost<User>("/login", { email: email.trim(), password }),
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (!email.trim() || !password) return; mutation.mutate(); }
  ...
  async function submitForgotPassword(event: FormEvent) {
    ...
    try {
      ...
      toast.error(axios.isAxiosError(error) ? error.response?.data?.detail || "Permintaan reset password gagal." : "Permintaan reset password gagal.");
