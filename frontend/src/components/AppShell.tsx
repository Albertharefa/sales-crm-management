...
const roleAllowedPrefixes: Record<string, string[]> = {
  ...
  SALES: ["/customers", "/pipeline", "/activities", "/quotations", "/purchase-orders", "/order-monitoring", "/sales-targets", "/products", "/settings"],
};
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const visibleGroups = navGroups.map(group => ({ ...group, items: group.items.filter(item => { if (item.label === "Users") return user.role === "SUPER_ADMIN" || user.role === "SALES_MANAGER"; if (item.label === "Audit Log" || item.label === "Sales Team") return user.role === "SUPER_ADMIN" || user.role === "SALES_MANAGER"; return true; }) })).filter(group => group.items.length > 0);
