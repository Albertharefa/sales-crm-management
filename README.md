...
# Sales CRM Management — Production Edition

This package is prepared for a single-service Railway deployment:
**React/Vite frontend + FastAPI backend + external MongoDB**.
...
- `/` — premium CRM web application
- `/login` — authentication
- `/api/*` — frontend API contract
- `/docs` — Swagger API documentation
- `/redoc` — ReDoc
- `/health` — Railway health check

The backend also keeps the clean legacy endpoints such as `/customers`,
`/pipeline`, `/quotations`, etc. They remain visible in Swagger. The `/api`
aliases are hidden from Swagger and point to the same handlers.

## Railway deployment

1. Push the contents of this project to GitHub.
2. Create/connect the Railway service to that repository.
3. Railway will detect `railway.toml` and build the root `Dockerfile`.
4. Add these Railway Variables:
   ...
   - `ADMIN_PASSWORD`
   - `COOKIE_SECURE=true`
   - `CORS_ORIGINS` (leave empty for same-origin production)
   - `OPENAI_API_KEY` (optional, for AI Copilot)
   - `OPENAI_MODEL` (optional)
5. Deploy and wait for the health check to pass.
6. Open the generated Railway domain. It should show the CRM login/dashboard,
   not the raw JSON API response.
...
The development Vite proxy sends `/api/*` to `http://localhost:8001`.
For local HTTP development the backend uses `COOKIE_SECURE=false` when
`APP_ENV` is not `production`.

## Production hardening included

- Single-origin frontend/API deployment.
- `/api` compatibility layer for the existing frontend and automated tests.
- SPA fallback for React Router refreshes.
- Explicit production router imports so missing modules fail fast.
- MongoDB connection timeouts and pool settings.
- Non-blocking database index initialization.
- 7-day TTL for session records.
- httpOnly session cookie with configurable Secure flag.
- RBAC dependency for Super Admin endpoints.
- Password hash excluded from public user responses.
- API errors normalized by the frontend fetch layer.
- Dedicated 404 page.
