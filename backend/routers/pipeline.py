...
from models.crm import Opportunity, OpportunityCreate, OpportunityUpdate, Paginated
...
async def validate_sales_assignment(sales_id: str, user: dict) -> dict:
    ...
    if role not in {"SUPER_ADMIN", "SALES_MANAGER", "SALES"}:
        raise HTTPException(status_code=403, detail="Role Anda tidak memiliki akses ke Sales Pipeline")
...
async def kanban(
    ...
    visible_ids = await visible_sales_ids(user)
    if sales_id and sales_id not in visible_ids:
        raise HTTPException(status_code=403, detail="Sales tersebut berada di luar scope Anda")

    base_filters: dict = {"sales_id": {"$in": visible_ids}}
    if sales_id:
...
async def update_opportunity(
    ...
    if "probability" in data and data["probability"] is not None:
        ...
        if probability < 0 or probability > 100:
            raise HTTPException(status_code=422, detail="Probability harus 0 sampai 100")

    if not data:
        return Opportunity(**doc)

    data["updated_at"] = now()
    await db.opportunities.update_one({"id": opportunity_id}, {"$set": data})
    updated = {**doc, **data}
    await audit(user, "Update", "Sales Pipeline", opportunity_id, data)
...
async def delete_opportunity(
    ...
    return {"message": "Opportunity berhasil dihapus", "id": opportunity_id}
